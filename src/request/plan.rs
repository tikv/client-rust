...
mod test {
    ...
    fn retryable(key_error: bool) -> RetryableMultiRegion<FailingPlan, MockPdClient> {
        RetryableMultiRegion {
            ...
            backoff: Backoff::no_backoff(),
            preserve_region_results: false,
            // The ordinary `retry_multi_region` defaults: `FailingPlan` answers with a
            // key or region error, never an undetermined apply outcome or a dispatch
            // failure, so neither opt-in is reachable here. What this test pins is the
            // error CLASS `ExtractError` reports, which is orthogonal to both.
            terminal_on_undetermined: false,
            terminal_on_dispatch_error: false,
        }
    }
