...
impl RetryClient<Cluster> {
    ...
    async fn do_get_region(self: Arc<Self>, key: Key) -> Result<Region> {
        retry!(self, "get_region", |cluster| {
            let key = key.clone();
            async {
                cluster
                    .get_region(key.clone().into(), self.timeout)
                    .await
                    .and_then(|resp| {
                        region_from_response(resp, || Error::region_for_key_not_found(key.into()))
                    })
            }
        })
    }
    ...
    pub async fn do_get_region_by_id(self: Arc<Self>, id: RegionId) -> Result<Region> {
