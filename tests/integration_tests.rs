...
async fn txn_get_timestamp() -> Result<()> {
    ...
    let client = TransactionClient::<transaction::ApiV1>::new(pd_addrs(), None).await?;
...
async fn raw_bank_transfer() -> Result<()> {
    init().await?;
    let client = RawClient::<raw::ApiV1>::new(pd_addrs(), None).await?;
...
async fn txn_pessimistic_rollback() -> Result<()> {
    init().await?;
    let client = TransactionClient::<transaction::ApiV1>::new_with_config(
        pd_addrs(),
        Default::default(),
        None,
    )
...
async fn raw_cas() -> Result<()> {
    init().await?;
    let client = RawClient::<raw::ApiV1>::new(pd_addrs(), None)
        .await?
        .with_atomic_for_cas();
